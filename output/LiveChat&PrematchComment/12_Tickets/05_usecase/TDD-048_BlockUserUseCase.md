...
實作 BlockUserUseCase。 / Implement BlockUserUseCase。

## 需求 / Requirements

1. 實作 `BlockUserUseCase` struct / Implement `BlockUserUseCase` struct
2. 定義 Input/Output Model / Define Input/Output Model
3. 整合 `LiveChatRepository` 呼叫 / Integrate `LiveChatRepository` call
4. 實作將用戶加入黑名單 DB 的邏輯 / Implement add user to blacklist DB logic
5. 實作 WebSocket 重連後同步 blocked user 清單的邏輯 / Implement WebSocket reconnect sync blocked user list logic
6. 實作 Error Handling / Implement Error Handling
7. 檔案結構：`Sources/LiveChat/UseCases/BlockUserUseCase.swift` / File structure: `Sources/LiveChat/UseCases/BlockUserUseCase.swift`
...
- [ ] `BlockUserUseCase` 實作完成 / `BlockUserUseCase` implementation complete
- [ ] Input/Output Model 定義完成 / Input/Output Model definition complete
- [ ] 黑名單管理邏輯實作完成 / Blacklist management logic implementation complete
