...
- **處理路由跳轉邏輯**（例如跳轉到個人主頁、跳轉到其他 Feature）
- 回傳 Domain Model 或純 Result 給 Feature
- 在序列圖中，以 participant（淡金色）呈現

**路由跳轉規則**：
- ✅ **UseCase 負責路由跳轉**：所有路由跳轉邏輯應由 UseCase 處理，而不是在 Feature 層直接處理
- ✅ **透過 Protocol 執行跳轉**：UseCase 應透過 Adapter Protocol（如 `<PersonalPageAdapter>`）來執行路由跳轉
- ✅ **Protocol 定義在 Package 中**：Adapter Protocol 定義在 Feature Package 中，實現放在 Main App 中
- ❌ **Feature 不應直接處理路由**：Feature 層不應直接調用路由方法，應透過 UseCase 處理
