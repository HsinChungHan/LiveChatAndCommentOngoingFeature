...
4. **Package 架構**：
   - LiveChat Package（內部 Package）和 FactsCenter Package(external)（外部 Package）都屬於 App 層級
   - FactsCenter Package(external) 是外部 Package，視為黑盒，不展示內部模組
   - LiveChat Package 是內部 Package，需要完整展示內部架構（Feature、UseCase、Repository、Client、API）
