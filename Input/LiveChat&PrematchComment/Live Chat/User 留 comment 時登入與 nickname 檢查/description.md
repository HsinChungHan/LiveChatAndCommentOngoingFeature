# 用戶留 comment 時登入與 nickname 檢查流程說明

## 流程概述

本流程為子流程（Sub Flow），主流程為：**用戶進入與離開聊天室（含 WebSocket 相依）**（flow_id: LC-FULL-001）。

本文件描述用戶在聊天室中輸入並送出留言時的完整流程，包括登入檢查、nickname 檢查、留言送出以及錯誤處理等情境。本流程涉及以下參與者：**User（用戶）**、**LiveChat Package（聊天室套件）**、**Main App（主應用程式）**、**PersonalPage Package(external)（個人頁面套件）**、**FComSharedFlow Package(external)（共享流程套件）** 和 **Server（伺服器）**。

---

## 1. 點擊輸入框與登入檢查

當用戶點擊輸入框準備輸入留言時，應用程式會先檢查用戶是否已登入。
